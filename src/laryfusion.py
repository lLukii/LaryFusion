...
class FocalLoss(nn.Module):
    """
    Focal loss for class imbalance. (Fine-tuned hyperparameters)
    """
    def __init__(self, gamma=2.0, alpha=0.98):
        super().__init__()
        self.gamma = gamma
        self.alpha = alpha
...
def eval_epoch(loader, model: FusionModule, loss_fn: FocalLoss):
    ...
    return total_loss / len(loader), correct / total, sensitivity, specificity, bal_acc, all_preds, all_labels
...
def get_model(num, dinput_dim): 
    ...
    match num: 
        ...
        case 2: model = FusionModule(config, dinput_dim, gate=False)
        case 3: model = Wav2VecBase(config)
        case 4: model = Wav2VecBase(config, freeze=True)
        case 5: model = FusionModule(config, dinput_dim, gate=True)
...
def main():
    ...
    accuracy/sensitivity/specificity/balanced accuracy across folds.
    """
    args = parse_args()

    print("Loading dataset...")
    dataset = load_patients(feature_type="wav2vec2", include_demographics=True, augment=args.model_type==1)
    ...
    for fold, (train_data, test_data) in enumerate(stratified_kfold(dataset, n_splits=args.n_splits), start=1):
        ...
        model = get_model(args.model_type, dinput_dim)
        loss_fn = FocalLoss().to(config.device) if args.model_type == 1 else \
            nn.CrossEntropyLoss(weight=torch.tensor([1., 32.], device=config.device))
        ...
        for epoch in range(1, config.num_epochs + 1):
            ...
            val_loss, val_acc, val_sens, val_spec, val_bal_acc, preds, labels = eval_epoch(
            ...
            print(f"Epoch {epoch:02d} | "
                  ...
                  f"specificity: {val_spec:.3f} bal_acc: {val_bal_acc:.3f}")
